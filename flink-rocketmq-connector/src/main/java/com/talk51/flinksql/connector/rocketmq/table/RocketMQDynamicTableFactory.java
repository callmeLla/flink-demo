...
/**
 * @ClassName: RocketMQDynamicTableFactory
 * @Description: 自定义工厂，将catalog的metadata转换为具体的连接器实例。
 * @Author: liujianing
 * @Date: 2021/4/7 15:12
 * @Version: v1.0 文件初始创建
 */
public class RocketMQDynamicTableFactory implements DynamicTableSourceFactory, DynamicTableSinkFactory {
    ...
    /** 连接ip */
    public static final ConfigOption<String> nameserver = ConfigOptions.key("nameserver").stringType().noDefaultValue();
    /** 主题 */
    ...
    public DynamicTableSink createDynamicTableSink(Context context) {
        ...
        final String nameserverVal = options.get(nameserver);
...
//        helper.validate();
        ...
        return new RocketMQDynamicTableSink(decodingFormat, producedDataType, nameserverVal, topicvalue, groupvalue, tagvalue);
    }
...
//        helper.validate();
        /** 数据类型设置 */
        final DataType producedDataType = context.getCatalogTable().getSchema().toPhysicalRowDataType();
        return new RocketMQDynamicTableSource(decodingFormat, producedDataType, nameserverVal, topicvalue, groupvalue, tagvalue);
    }
